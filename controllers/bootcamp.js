...
exports.getBootcamp=asyncHandler(async(req, res,next)=>
{
    ...
    if(!bootcamp)
    {
        return next(new ErrorResponse(`Bootcamp ${req.params.id} doesn't exists `, 404))
    }
    res.status(200).json(
    {
        success:true, 
...
exports.createBootcamp=asyncHandler(async(req, res,next) =>
{
	 ...
	 */
	if ((!publishedBootcamp && req.user.role == 'publisher') || req.user.role == 'admin') 
	{
	...
	if( req.user.role == 'user')
	{
		return next(
			new ErrorResponse(
			  `The user with ID ${req.user.name} has not permission to publish a bootcamp`,
			  400
			)
		);
	}
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
	...
	if(req.user.role == 'admin' || (bootcamp.user.toString() == req.user.id))
	{
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
	...
	else
	{
		return next(
			new ErrorResponse(
			  `User ${req.user.name} is not authorized to upload image`,
			  401
			));
	}
